...
import {ImageStateType, ImageType, isErrorMessage} from '@/types/mainPageTypes';
...
export const HomeComponent = ({
  ...
  const [images, setImages] = useState<Array<ImageStateType>>(imagesProps);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [selectedImages, setSelectedImages] = useState<
    Array<{id: number; index: number}>
  >([]);
  const [isSelectMode, setIsSelectMode] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(reqError);
