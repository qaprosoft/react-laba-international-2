...
function offset(date) {
  ...
  const correctFormatDate = new Date(
    year,
    month - 1,
    day,
    hour,
...
function randomDate(date1, date2) {
  ...
  const millisecondsBetweenTwoDates = Math.abs(
    correctFormatDate2 - correctFormatDate1,
  );
  const randomDayInMillisec = Math.floor(
    Math.random() * millisecondsBetweenTwoDates,
  );
