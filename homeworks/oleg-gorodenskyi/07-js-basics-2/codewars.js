...
function decipherThis(str) {
  ...
  const parsedArrayOfWords = arrayOfWords.map((word, index) =>
    word.replace(numbers[index], parsedNumbers[index]),
  );

  const result = parsedArrayOfWords.map(word => {
    let wordArray = word.split('');
    wordArray.length > 2
      ? ([wordArray[1], wordArray[wordArray.length - 1]] = [
...
function sortArray(array) {
  ...
  array.map(num =>
    num % 2 !== 0 ? result.push(sortedOddArr.shift()) : result.push(num),
  );
  return result;
}
