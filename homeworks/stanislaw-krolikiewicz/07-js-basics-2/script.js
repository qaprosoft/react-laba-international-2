...
const positiveSum = arr =>
  arr.reduce((sum, current) => (current > 0 ? sum + current : sum), 0);
...
const gimme = t => {
  ...
  else if ((t[0] < t[1] && t[1] < t[2]) || (t[2] < t[1] && t[1] < t[0]))
    return 1;
...
const binaryArrayToNumber = arr =>
  arr.reverse().reduce((sum, curr, index) => sum + (2 * curr) ** index);
...
const decipherThis = str => {
  ...
  arr.forEach(el => {
    ...
    let substitute = letters.length
      ? letters.length > 1
        ? letters[letters.length - 1] +
          letters.substring(1, letters.length - 1) +
          letters[0]
        : letters[0]
      : '';
...
const findUniq2 = arr => {
  ...
  for (let i = 0; i < first.length; i++) {
    if (!letters.find(el => el.toLowerCase() === first[i].toLowerCase()))
      letters.push(first[i].toLowerCase());
  }
