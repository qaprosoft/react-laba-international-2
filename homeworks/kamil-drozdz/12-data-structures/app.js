...
const MOCK_DATA = require('../../../lectures/12-data-structures-and-algorithms/MOCK_DATA');
...
const straightSearch = (mockData, skuToFind) => {
  return mockData.find(item => item.sku === skuToFind) || -1;
};
...
// Sorting the data
const sortedData = quickSort([...MOCK_DATA]);

// Test
console.log(straightSearch(MOCK_DATA, skuForTests)?.sku === skuForTests);
...
const perfomanceTest = (callback, array) => {
  ...
  const message = `${callback.name} test took ${duration} milliseconds to execute ${
...
perfomanceTest(straightSearch, MOCK_DATA);
