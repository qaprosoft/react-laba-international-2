...
function offset(date){
    ...
    let millsPerHour = 3.6e+6;
    let millsPerDay = 8.64e+7;
    let millsPerYear = 3.154e+10;
    let millDifference = Date.now() - fullDate.getTime();

if(millDifference>millsPerYear){
    return   "the date was " + (millDifference/millsPerYear).toFixed(2) + " years ago"
} else if( millDifference>millsPerDay){
    return "the date was " + (millDifference/millsPerDay).toFixed(2) + " days ago"
} else {
    return "the date was " + (millDifference/millsPerHour).toFixed(2) + "hours ago"
}   
}
...
function randomDate(date1, date2){
    ...
    return date2Milliseconds>date1Milliseconds? new Date(Math.random()*(date2Milliseconds-date1Milliseconds)+date1Milliseconds) : new Date(Math.random()*(date1Milliseconds-date2Milliseconds)+date2Milliseconds);
...
function NamedOne(first, last) {
    ...
    this.last = last;
      Object.defineProperties(this, {
             ...
             set: function(first) { this.first = first;}
          },
              "lastName": {
             get: function() { return this.last; },
             set: function(last) { this.last = last; }
          },
              "fullName": {
              get: function () { return this.first + ' ' + this.last },
              set: function (fullName) {
                
                if(fullName.search(/[^a-zA-Z' ']/g)&&fullName.split(' ').length>1){
