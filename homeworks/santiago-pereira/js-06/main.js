...
function solution15(word) {
  const lowerCaseWord = (word = word.toLowerCase()); // Convert the word to lowercase to ignore capitalization
  let result = '';

  for (let i = 0; i < lowerCaseWord.length; i++) {
    if (lowerCaseWord.split(lowerCaseWord[i]).length - 1 > 1) {
      result += ')';
    } else {
      result += '(';
    }
...
function solution17(floor) {
  ...
  for (let i = 0; i < floor; i++) {
    ...
    while (counter < maxSize) {
      if (
        counter >= Math.floor(maxSize / 2 - i) &&
        counter <= Math.floor(maxSize / 2 + i)
      ) {
        str += asterisk;
      } else {
...
function solution18(str) {
  ...
  for (let i = 0; i < lowerStr.length; i++) {
    if (lowerStr[i] !== ' ') {
      ...
      const finalString =
        lowerStr.slice(0, i) + capitalized + lowerStr.slice(i + 1);
